...
const postProduct = async (name, price, amount, farmer_username, description, picture) => {
...
const getProductById = async (id) => {
  //Unnecessary function as the getProducts function can accept an id by itself and have the same result
  const query = knex(PRODUCT_TABLE).where("id", id);
  const result = await query;
  return result;
};

const getProducts = async (id, name, amount, price) => {
  //This returns products based on a dynamic search query that can include
...
const getProductByTag = async (tag) => {
  const query = knex(PRODUCT_TABLE).modify(function (joinquery) {
    if (tag) {
